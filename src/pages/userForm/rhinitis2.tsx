...
import questionsData from './mock/rhinitis2.json'
import './index.less'
...
interface UserFormState {
  // 核心 4
  isHDMA: string
  isOtherAllergy: string[]
  conditionCatOrDogAppear: string
  antiAllergicMedicine: string
  // 其他 3
  isAgeRange: string
  sex: string
  conditionFemalePregnant: string
  // 病史 9
  hasIllness1: string[]
  hasIllness2: string[]
  hasIllness3: string[]
  hasRareDiseases: string[]
  hasInfection: string[]
  hasAdverseReaction: string[]
  hasOtherAllergy: string
  hasEosinophilic: string
  hasGeneralizedAllergic: string[]
  // 治疗史 1
  antipsychotics: string
  // 洗脱 11  (18-)
  desensitization: string
  immunosuppressor: string[]
  ketotifen: string
  hasRTI: string[]
  twoYears: string[]
  nasalOperation: string
  fiveyears: string
  takeGlucocorticoid: string[]
  injectionGlucocorticoid: string[]
  emergencyMedical: string[]
  antiAllergicBiologicalAgents: string[]
  // 承诺 0
  // 停用药 5
  betaBlocker: string[]
  immunosuppressorNow: string
  biologicals: string
  antihistamineDrug: string
  iathergy: string
...
export default class UserForm extends Component<{}, UserFormState> {
  state: UserFormState = {
    isHDMA: '', // 核心 4
    isOtherAllergy: [],
    conditionCatOrDogAppear: '',
    antiAllergicMedicine: '',
    isAgeRange: '', // 其他 3
    sex: '',
    conditionFemalePregnant: '',
    hasIllness1: [], // 病史 9 
    hasIllness2: [],
    hasIllness3: [],
    hasRareDiseases: [],
    ...
    hasOtherAllergy: '',
    hasEosinophilic: '',
    hasGeneralizedAllergic: [],
    antipsychotics: '', // 治疗史 1
    desensitization: '', // 洗脱 11
    immunosuppressor: [],
    ketotifen: '',
    hasRTI: [],
    twoYears: [],
  ...
  getExitConditions = () => {
    return {
      ...
      isOtherAllergy: ['其他过敏原'],
      conditionCatOrDogAppear: ['否'],
      isAgeRange: ['否'],
      conditionFemalePregnant: ['是'],
      conditionFemaleOther: ['不接受'],
      limitDuring: ['不接受'],
      selfRelationship: ['不符合'],
      agreeCheckUp: ['拒绝检查'],
      hasIllness: ['不受控的哮喘','除哮喘外其他肺病','心脏病','除心脏病外其他心血管疾病','高血压','甲亢','肿瘤','艾滋病','除鼻炎/鼻结膜炎外的鼻部疾病','严重口腔疾病','精神疾病'],
      hasInfection: ['HIV','乙肝','梅毒','真菌','寄生虫'],
      hasAdverseReaction: ['食物严重不良反应','蜜蜂、黄蜂等叮咬后严重不良反应','药物严重不良反应'],
      hasRareDiseases: ['有自身免疫病（如红斑狼疮、桥本甲状腺炎）','免疫力严重低下（如器官移植后，长期使用免疫抑制剂等）','有遗传性血管性水肿疾病'],
      corticosteroidsUsed: ['是'],
      twoYears: ['慢性荨麻疹','严重速发过敏反应（如过敏性休克）'],
      threeMonths: ['糖皮质激素（如甲强龙）','任何生物制剂药物（例如单抗）','任何免疫抑制剂'],
      fourWeeks: ['呼吸道感染','哮喘急性发作且病情不稳定'],
      twoWeeks: ['是'],
